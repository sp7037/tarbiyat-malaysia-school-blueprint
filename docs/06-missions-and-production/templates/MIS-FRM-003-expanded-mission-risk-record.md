# MIS-FRM-003 — Expanded Mission Risk Record

| Field | Value |
|---|---|
| Template ID | `MIS-FRM-003` |
| Version | `0.1.0` |
| Status | `DRAFT` |
| Classification | **CONDITIONAL RECORD** — required for elevated Missions / work packages (typically R2+ or when FRM-001 basic screen triggers expansion) |
| Last reviewed | `2026-09-15` |
| Country scope | `CORE` |
| Owner / reviewer | Mission Lead + competent safety / safeguarding functions (MIS-003 §26) |
| Implements | MIS-003 §§3–27 risk register concept |
| Related gaps | GAP-031; GAP-024 |

> **Operational educational record — not a substitute for country-required legal documentation.**  
> **R-level classifies the activity / work package — not the learner.** It is **not** a prestige score (MIS-003 §2).  
> Forms cannot waive R4 prohibitions (clinical practice, unauthorised live cyber, unmanaged exploitation).

---

## Instance control

| Field | Entry |
|---|---|
| Related Mission ID / title | |
| Specific activity / work package | |
| Linked MIS-FRM-001 (if partner Mission) | |
| Date | |
| Record owner | |
| Approver(s) | |
| Review / re-review date | |

---

## Classification

| Field | Entry |
|---|---|
| Risk domains present (list MIS-003 domain IDs A–O) | |
| R-level (R0–R4) | Highest applicable for this package |
| Overall Mission ceiling (if multiple packages) | Highest active package |
| Authenticity level (A–F) if known | Separate axis — authenticity ≠ risk |
| Hazards (plain language) | |
| Why this R-level | |

---

## Controls

| Field | Entry |
|---|---|
| Required controls | Supervision / specialist / PPE / equipment / data / consent / partner / stage gates / simulation / sandbox / deployment cap / communication / financial / other (MIS-003 §7) |
| Supervision model | |
| Competent adult / function responsible | |
| PPE / environment requirements | Implementation detail — no Core catalogue invented |
| Data restrictions | Align MIS-001 / ADR-0007 classes; link MIS-FRM-005-B if needed |
| Equipment restrictions | |
| Learner competence prerequisites | Induction / tool demo / supervised practice as proportional — not a badge farm |
| Simulation / sandbox requirements | |
| Deployment cap (ADR-0007 mode) | e.g. educational prototype only until MIS-FRM-006-A |
| Partner interaction limits | Partners inform; do not unilaterally approve (MIS-003 §11) |

---

## Residual risk and authority

| Field | Entry |
|---|---|
| Residual risk after controls | |
| Stop-work authority holders | School functions; partner has no unilateral override |
| Approval / review status | |
| Re-review triggers | New partner/role; deployment; new data class; equipment change; money; scope creep; incident (MIS-003 §21) |
| Incident / near-miss reference | |

---

## Hard boundaries (cannot be waived)

| Check | Status |
|---|---|
| Unlicensed clinical practice / diagnosis / invasive procedures | ☐ Not in scope / ☐ Redesigned to simulation or non-clinical |
| Highly Restricted raw clinical / classified access for learners | ☐ Not in scope / ☐ Reduced / refused |
| Unauthorised offensive testing of live external systems | ☐ Not in scope |
| Production quotas / free labour substitution | ☐ Not in scope |
| AI/software used to autonomously approve this activity | ☐ Not used for approval |

---

## Decision

| Outcome | Tick |
|---|---|
| Proceed with documented controls | ☐ |
| Proceed only in simulation / sandbox | ☐ |
| Restrict stage / participants | ☐ |
| Cap deployment mode | ☐ |
| Redesign / defer | ☐ |
| Prohibited as framed (R4) | ☐ |

---

## Explicit non-claims

This template is **not** a database schema, PPE catalogue, country incident code, or learner risk score.
