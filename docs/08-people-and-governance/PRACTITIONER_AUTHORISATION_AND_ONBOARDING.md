...
This document proposes resolution of **GAP-033**. It is **not** a Mission Team loading / staffing capacity standard (`GAP-032` — see `PEO-004`), currency programme (`GAP-035` — see `PEO-002`), faculty development standard (`GAP-036` — see `PEO-003`), or country registration map (`GAP-037`).
...
Numeric supervision **ratios** are not set here (`GAP-032` / `PEO-004` capacity principles).
