...
| Source documents | Ratification (2026-09-13); Iteration 2–13 ADR drafts; Integrated Developmental Architecture approvals; ADR-0007 APPROVED / GAP-018 Closed (2026-09-17); ADR-0015 APPROVED (2026-09-19); ADR-0014 APPROVED / GAP-025 Closed (2026-09-19) |
...
> UNDER REVIEW decision ADR count = 0. Remaining work is operating / country / pilot / templates / software / business — not further Core ADR under-review backlog.
...
| ADR-0014 | Student Creator Rights, Intellectual Property, Innovation and Micro-Venture Architecture | `APPROVED` | `1.0.0` | `docs/13-decisions/adr/ADR-0014-student-creator-rights-ip-innovation-micro-venture.md` |
...
| ADR-0020 | Campus and Facilities Functional Architecture | `APPROVED` | `1.0.1` | `docs/13-decisions/adr/ADR-0020-campus-and-facilities-functional-architecture.md` |
...
*(None — all decision ADRs currently in body-file review are APPROVED.)*
...
**ADR-0007** (`APPROVED` v1.0.0) defines governed Mission Ecosystem; Mission vs Activity/Project; authenticity levels A–F; Strategic Capability Missions; Institution-in-Residence; Education-First / no exploitation; Mission Educationalisation and lifecycle; Campus Live Workflow; Deployment Boundary. Depends on FND-003 / ADR-0001–0006 / 0011 / 0016 / 0019 / 0020 (APPROVED); ADR-0014 / ADR-0015 now APPROVED. Does **not** invent Malaysian partnerships, IP ownership, PoC ops schemas or strategic-sector Core lists. **Closes `GAP-018`.** MIS-001 / MIS-002 / MIS-003 APPROVED (`GAP-022` / `023` / `024` Closed).
...
**ADR-0014** (`APPROVED` v1.0.0) defines Student Creator Rights; educational recognition vs legal ownership (authorship, contribution, ownership, possession, licence, publication, commercial rights distinguished); outcome classification; Contribution Records; Background/Mission/Joint IP; Surprise Innovation; Creator Rights Review; optional Micro-Venture; benefit-sharing principles without Core %; no automatic IP transfer; enrolment ≠ unlimited promotional consent. Depends on FND-003 / ADR-0001–0007 / 0011 / 0015 / 0016 / 0019 / 0020 (APPROVED). Does **not** invent country IP law, ownership percentages, contracts or PoC schemas. **Closes `GAP-025`.** Downstream: `GAP-026`–`GAP-031`; MY feasibility remains `GAP-011`. MIS/POC/MY/IMP remain DRAFT.

**ADR-0015** (`APPROVED` v1.0.3) defines Practitioner–Educator–Mentor as peer faculty **functions** (not tiers) inside professional learning teams; Technical/Lab Professionals as supporting workforce (not a fourth faculty function); Specialist Services kept distinct from Mentor; proportional Mission Teams; Mission Educationalisation shared responsibility; multi-stream assessment; pathway non-monopoly; practitioner currency / educator development / practitioner onboarding / mentor preparation principles; Institution-in-Residence ≠ automatic faculty; authorisation asymmetry (Practitioner scoped PA ≠ Educator/Mentor Core PA ladders). Depends on FND-003 / ADR-0001–0007 / 0011 / 0014 / 0016 / 0019 / 0020 (APPROVED). Does **not** set ratios, invent country registration law or close remaining ops gaps. `PEO-001` / `MIS-FRM-007` **APPROVED** (GAP-033 Closed). `PEO-002` / `MIS-FRM-008` **APPROVED** (GAP-035 Closed). `PEO-003` / `PEO-FRM-001` **APPROVED** (GAP-036 Closed). `PEO-004`–`PEO-005` remain **DRAFT** proposed resolutions of `GAP-032` / `034` — **not closed**. `GAP-037` via `MY-002` DRAFT — **not closed**.

**ADR-0016** (`APPROVED` v1.0.0) defines evidence-backed human-governed Proof of Capability Profile; Capability Claims; Proof ≠ Participation; Direct/Supporting/Context evidence; Contribution Records; independence context (descriptive); productive failure; multi-source verification; Capability Defence (proportional); AI disclosure without banning AI; provenance; Sanitised Verified Evidence; longitudinal growth without permanent deficit labels; no universal composite score; AI non-certification; Competency Graph kept distinct (`GAP-009`); Six Worlds evidence parity; workload practicality. Depends on ADR-0003 (APPROVED) and ADR-0007 / 0015 / 0014 (APPROVED); ADR-0004–0006 portability (APPROVED). Does **not** claim legal qualification status, invent Graph schemas or blockchain. **Closes `GAP-010`.** Operating companions `POC-001` / `POC-FRM-*` remain DRAFT; `GAP-038`–`GAP-044` proposed / Open / Partial — **not closed** (`GAP-043` research remains Open).
