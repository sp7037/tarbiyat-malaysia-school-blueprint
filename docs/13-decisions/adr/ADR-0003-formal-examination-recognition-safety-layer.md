...
| Source documents | Architecture brief (2026-09-13); Malaysia Concept Note (exams as safety net — proposal language); Oman v3 and early 180° drafts (exam-abolition language — superseded as current direction) |
...
- The current founding brief requires national/board examination compatibility and dual-layer architecture.
...
Where the country of implementation requires a recognised curriculum and formal examinations (or equivalent statutory assessment), those requirements **remain in place**. Tarbiyat Core design must remain compatible with that layer. Exact boards, subjects and regulations are defined only in the country implementation profile after authoritative research.

**Recognised examinations are not abolished.**

Historic source statements advocating abolition of examinations are **superseded as current design direction**.
...
This layer evidences what students can investigate, design, build, improve and present. It is **additional** to formal recognition. It must not be described as replacing statutory requirements.

### 3. Joint graduation intent

By graduation, students should possess **both** formal examination/recognition credentials required locally **and** a Tarbiyat longitudinal capability record, subject to country rules and the student’s pathway stage.
...
- Risk that examination preparation crowds out missions unless ADR-0002 timetabling is solved.
...
- Core assessment specification (`docs/05-assessment/`) after this ADR’s ratification path is clear.
- Claims Register updates: CLM-002 treated as superseded design direction pending human ratification of this ADR.
...
4. **Numeric split mandated in Core** (e.g. 40% board / 60% missions from historical blending draft). Rejected for now: may be a later country or timetable decision; not elevated without evidence and ADR (CLM-007 remains unverified proposal).

## Country-Specific Implications

| Topic | Core | Country profile |
|---|---|---|
| Whether formal exams exist | Assume compatibility with recognised systems where required | Confirm local legal reality with citations |
| Which board/curriculum | Not decided in Core | Malaysia (and others): research and map (`GAP-002`) |
| Exam preparation time | Must be accommodated in Learning Engine design | Local calendar and subject rules |
| Recognition of Proof of Capability by universities/employers | Permitted as aspiration only | Requires local evidence; no invented endorsements |
...
4. What happens for students who meet capability expectations but underperform on statutory exams (support model TBD)?
