...
| Dependencies | `FND-003` APPROVED; `ADR-0001` APPROVED; `ADR-0002` APPROVED; `ADR-0003` APPROVED; `ADR-0004`–`ADR-0006` APPROVED; `ADR-0007` APPROVED; `ADR-0011` APPROVED; `ADR-0014` APPROVED; `ADR-0016` APPROVED; `ADR-0019` APPROVED; `ADR-0020` APPROVED; operational companion `PEO-001` APPROVED (GAP-033 Closed); `PEO-002` APPROVED (GAP-035 Closed); `PEO-003` APPROVED (GAP-036 Closed); `PEO-004`–`PEO-005` DRAFT (do not close GAP-032 / 034); `MIS-001`–`MIS-003` APPROVED; `MIS-FRM-002` / `MIS-FRM-007` APPROVED; `MIS-FRM-008` APPROVED |
| Related gaps / claims | GAP-033 Closed by PEO-001 / MIS-FRM-007; GAP-035 Closed by PEO-002 / MIS-FRM-008; GAP-036 Closed by PEO-003 / PEO-FRM-001; GAP-032 / 034 proposed via PEO-004 / 005 DRAFT — not closed; GAP-037 proposed via MY-002 DRAFT — not closed; related GAP-005, GAP-022, GAP-023 |
...
| 1.0.3 | 2026-09-23 | Status/dependency hygiene only: `PEO-003` / `PEO-FRM-001` APPROVED (GAP-036 Closed); `PEO-004`–`005` remain DRAFT; no substantive faculty-architecture change |
...
Before this ADR, `docs/08-people-and-governance/` was a section shell and no Core staffing ADR existed. Historical proposals mentioned coaches, mentors and academic staff without an operable faculty architecture. **People operating standards** under `SEC-08` implement this ADR’s principles: `PEO-001` is **APPROVED** (GAP-033 Closed); `PEO-002` is **APPROVED** (GAP-035 Closed); `PEO-003` is **APPROVED** (GAP-036 Closed); `PEO-004`–`PEO-005` remain **DRAFT** and do **not** close GAP-032 / 034 by existing.
...
**Dependency note:** This ADR assumes ADR-0007’s Mission Educationalisation, Education-First and Institution-in-Residence boundary language (APPROVED), and ADR-0014’s Creator Rights / conflict principles (APPROVED). Developmental stage architecture from ADR-0004–0006 is APPROVED context. Country / ops Creator Rights detail remains open (`GAP-026`–`030`).
...
10. **Practitioner currency**, **Educator development**, **Practitioner onboarding** and **Mentor preparation** are Core architectural expectations. Practitioner onboarding / authorisation is operationalised by APPROVED `PEO-001` / `MIS-FRM-007` (GAP-033 Closed). Practitioner currency is operationalised by APPROVED `PEO-002` / `MIS-FRM-008` (GAP-035 Closed). Faculty / practitioner development and Mentor preparation development routes are operationalised by APPROVED `PEO-003` / `PEO-FRM-001` (GAP-036 Closed). Mentor caseload remains `PEO-005` (DRAFT; GAP-034 — **not closed**). Mission Team loading / staffing capacity principles are operationalised provisionally by `PEO-004` (DRAFT; GAP-032 — **not closed**).
...
Development architecture: `PEO-003` / `PEO-FRM-001` (APPROVED; `GAP-036` Closed). Course calendars and certificates remain implementation detail.
...
Mentor preparation sits within `PEO-003` (APPROVED; GAP-036 Closed). Caseload / human-guidance capacity: `PEO-005` / `PEO-FRM-003` (DRAFT; proposed resolution of `GAP-034` — **not closed**). No universal Core Mentor∶learner ratio is set.
...
- Hybrid talent scarcity until pipelines mature (development architecture now APPROVED via `PEO-003` / GAP-036 Closed; capacity / caseload remain open).
- Risk of documentation and meeting overload if teams are poorly designed (`GAP-032` / `PEO-004` DRAFT — not closed).
- Dependency on ADR-0014 remaining coherent through Creator Rights–sensitive faculty conflict procedures (ADR-0014 now APPROVED; ops detail remains country/ops follow-on).
- Country mapping still required before operational hiring (`GAP-037` Open).

### Follow-on work

- Human review of remaining DRAFT people operating standards; gaps stay open until PEO / country human review:
  ...
  - Faculty / Practitioner development — `PEO-003` / `PEO-FRM-001` (`GAP-036` Closed).
...
- ADR-0014 (APPROVED — Creator Rights; conflict of interest)
...
- Forms: `PEO-FRM-001` (APPROVED); `PEO-FRM-002`–`003` (DRAFT); `MIS-FRM-002` / `007` / `008` (APPROVED)
...
4. Faculty development programme design for Educators, Practitioners and Mentors — operationalised by APPROVED `PEO-003` / `PEO-FRM-001` (`GAP-036` Closed).
...
9. Re-check Creator Rights–sensitive faculty conflict procedures under APPROVED ADR-0014 as country/ops packs mature.
...
- treat `PEO-004`–`PEO-005` as approved law or as closing GAP-032 / 034 merely by existing (APPROVED `PEO-001` closes GAP-033; APPROVED `PEO-002` closes GAP-035; APPROVED `PEO-003` closes GAP-036 only);
