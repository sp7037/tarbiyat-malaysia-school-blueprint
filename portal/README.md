...
- **New Foundation / register / operational standard document:** add an entry to the `includes` list in `portal/scripts/prepare-content.mjs` and, if needed, a sidebar item in `portal/docs/.vitepress/config.mts`. Examples: `MIS-001` → `/areas/mission-partner-governance`; `PEO-001` → `/areas/practitioner-authorisation`; `PEO-002` → `/areas/practitioner-currency`; `MIS-TPL-001` → `/areas/mission-operating-templates` (individual `MIS-FRM-*` pages generated but kept off the main sidebar except via the templates index).
...
Mission Operating Templates: [index](/areas/mission-operating-templates). Practitioner chain: [PEO-001](/areas/practitioner-authorisation) → [PEO-002](/areas/practitioner-currency).
