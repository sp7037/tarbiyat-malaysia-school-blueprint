---
title: Student Journey
description: Architecture area — student journey (APPROVED Core stage architecture)
---

# Student Journey

<p class="present-more"><a href="/student-journey">← Stakeholder presentation: Student Journey</a></p>

<div class="planned-banner">
  <strong>Blueprint area — Core stage architecture APPROVED</strong><br/>
  Constitution names Explore→Discover→Choose→Deepen→Produce. ADR-0004–0006 are APPROVED.
  ADR-0017 Discover meaningful-exposure package APPROVED (GAP-045 Closed).
  ADR-0018 Discover→Choose pathway preparation APPROVED (GAP-046 Closed).
  Human Mentor capacity for pathway dialogue: [PEO-005](/areas/mentor-caseload-capacity) (GAP-034 proposed — not closed; ADR-0015 APPROVED; PEO-005 remains DRAFT).
  The dedicated section under <code>docs/02-student-journey/</code> remains a DRAFT shell for detailed specs.
</div>

::: tip Portal notice
Source Markdown documents remain authoritative. This page does not invent journey specifications.
:::

## What already exists

| Artefact | Status | Link |
|---|---|---|
| Explore→Discover→Choose→Deepen→Produce (Constitution) | APPROVED principle | [Concept Constitution](/foundation/concept-constitution) |
| Stage architecture | APPROVED | [ADR-0004](/decisions/adr-0004) |
| Stage windows / Major-Minor entry | APPROVED | [ADR-0005](/decisions/adr-0005) |
| Transition / Exploration Floor / mobility | APPROVED | [ADR-0006](/decisions/adr-0006) |
| Discover meaningful exposure | APPROVED | [ADR-0017](/decisions/adr-0017) |
| Discover→Choose pathway preparation | APPROVED | [ADR-0018](/decisions/adr-0018) |
| Mentor human-guidance capacity | DRAFT | [PEO-005](/areas/mentor-caseload-capacity) |
| Section shell | DRAFT | Repository `docs/02-student-journey/SECTION_README.md` |

## Related gaps

- [GAP-003](/gaps#gap-003) — Age bands / typical windows (**Closed** via ADR-0005)
- [GAP-004](/gaps#gap-004) — Major + Minor introduction (**Closed** via ADR-0005)
- [GAP-019](/gaps#gap-019) — Exploration Floor operationalisation (**Closed** via ADR-0006)
- [GAP-020](/gaps#gap-020) — Pathway change bridging (**Closed** via ADR-0006)
- [GAP-045](/gaps#gap-045) — Discover meaningful-exposure package (**Closed** via ADR-0017)
- [GAP-046](/gaps#gap-046) — Discover→Choose pathway preparation (**Closed** via ADR-0018)
- [GAP-034](/gaps#gap-034) — Mentor caseload / human-guidance capacity (proposed via [PEO-005](/areas/mentor-caseload-capacity) — not closed)
